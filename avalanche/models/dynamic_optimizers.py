...
def reset_optimizer(optimizer, model):
    ...
    """
    # DEBUG
    # print("len param groups", len(optimizer.param_groups))
    # for group in optimizer.param_groups:
    #     print("len params in group", len(group['params']))
    #     print(group)
    # END DEBUG

    # assert len(optimizer.param_groups) == 1 # We need to be sure that the first optimizer group is always the model.. but then this assert is not useful..

    optimizer.state = defaultdict(dict)
    optimizer.param_groups[0]['params'] = list(model.parameters()) # NOTE: param_groups[0] need to be the model's parameters
    print("\nRESET OPTIMIZER")
    return
