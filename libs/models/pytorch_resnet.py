"""
Copyright: https://pytorch.org/vision/0.8/_modules/torchvision/models/resnet.html
...
class ResNet(nn.Module):
    def __init__(self, block, layers, nf=64, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, dropout_rate=0.0, 
                 use_pooling="GAP", small_resolution_model=False):
        ...
        if small_resolution_model:
            self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=3, stride=1, padding=1,
                               bias=False)
            self.maxpool = nn.Identity()

        self.layer1 = self._make_layer(block, nf, layers[0], stride=1) # NOTE: not applying dropout here
        ...
        print("")
        print("Use pooling", use_pooling)
        print("")
        self.pooling_layer = None # NOTE defaults to using no pooling
        if use_pooling == "GAP":
...
def resnet18(**kwargs):
    ...
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], **kwargs)
                ...
                #    small_resolution_model=small_resolution_model) # NOTE: mind the BasicBlock

def resnet101():
    return _resnet('resnet101', Bottleneck, [3, 4, 23, 3])

def resNeXt50():
    return _resnet('resnext50', Bottleneck, [3, 4, 6, 3], groups=32, width_per_group=4)

def wide_resnet18(nf=64, widen_factor=2):
    return _resnet('resnet18', Bottleneck, [2, 2, 2, 2], nf=nf, width_per_group=int(widen_factor*nf)) # NOTE: mind the Bottleneck
