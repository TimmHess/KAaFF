...
class KNNPCAProbingAccuracyMetric(GenericPluginMetric[float]):
    def __init__(self, 
                 train_stream, 
                 test_stream, 
                 k, 
        ...
        super(KNNPCAProbingAccuracyMetric, self).__init__(
    ...
    def before_eval_exp(self, strategy: 'BaseStrategy'):
        ...
        for _, mbatch in tqdm(enumerate(lp_dataloader), total=len(lp_dataloader)):
                ...
                x_rep = strategy.model.forward_feats(x).detach()
                x_rep = x_rep.view(x_rep.shape[0], -1) # TODO: is this even necessary?
                x_rep = torch.nn.functional.normalize(x_rep, dim=1)
                x_rep = x_rep.cpu().numpy()

                # Reduce dim in head if requested
                if self.reduce_dim_in_head:
                    x_rep = self.pca.transform(x_rep)
                print("x_rep_concat.shape:", x_rep.shape)
